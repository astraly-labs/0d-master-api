...
impl KpiService {
    ...
    async fn wait_for_indexers_synced(&self) -> anyhow::Result<()> {
        ...
        loop {
            ...
            let indexer_states = self
                .db_pool
                .interact_with_context(
                    "fetch all indexer states".to_string(),
                    IndexerState::find_all,
                )
                .await
    ...
    async fn get_user_portfolio_history(
        ...
        let portfolio_history = self
            .db_pool
            .interact_with_context(
                format!("fetch portfolio history for user: {user_address}, vault: {vault_id}"),
                move |conn| {
    ...
    async fn get_active_vaults(&self) -> anyhow::Result<Vec<Vault>> {
        let vaults = self
            .db_pool
            .interact_with_context("fetch all live vaults".to_string(), Vault::find_live)
            .await?;
    ...
    async fn get_vault_user_positions(&self, vault_id: &str) -> anyhow::Result<Vec<UserPosition>> {
        ...
        let positions = self
            .db_pool
            .interact_with_context(
                format!("fetch user positions for vault: {vault_id}"),
                move |conn| UserPosition::find_active_by_vault(&vault_id_clone, conn),
            )
            .await?;
    ...
    async fn get_user_vault_transactions(
        ...
        let transactions = self
            .db_pool
            .interact_with_context(
                format!("fetch transactions for user: {user_address}, vault: {vault_id}"),
                move |conn| {
    ...
    async fn insert_daily_portfolio_history(
        ...
        self.db_pool
            .interact_with_context(
                format!(
                    "insert daily portfolio history for user: {user_address}, vault: {vault_id}"
                ),
                move |conn| {
    ...
    async fn insert_daily_user_kpis(
        ...
        self.db_pool
            .interact_with_context(
                format!("upsert user KPIs for user: {user_address}, vault: {vault_id}"),
                move |conn| UserKpi::upsert(&user_address, &vault_id, &kpi_data, conn),
            )
            .await?;
